...
const WhyShopWithUs = () => {
  ...
  return (
    <section className="py-20 px-6 max-w-7xl mx-auto">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
        {reasons.map((reason, index) => {
          ...
          return (
            <div
              key={index}
              className="group relative overflow-hidden rounded-2xl bg-[#111827] border border-gray-800 p-8 hover:border-cyan-500/50 transition-all duration-300"
            >
              {/* Background */}
              <div
              ...
              {/* Glow */}
              <div className="absolute -top-1/2 -right-1/2 w-80 h-80 bg-cyan-500/20 rounded-full blur-3xl opacity-0 group-hover:opacity-50 transition-all duration-500 group-hover:scale-150"></div>

              {/* Content */}
              <div className="relative z-10">
                {/* Icon */}
                <div className="w-14 h-14 bg-cyan-500/20 rounded-lg flex items-center justify-center mb-4 group-hover:bg-cyan-500/30 transition-all border border-cyan-500/30 group-hover:scale-110 transform duration-300">
                  <IconComponent
                    size={28}
      ...
      {/* Trust Badges */}
      <div className="mt-16 p-8 bg-gradient-to-r from-cyan-600/10 to-teal-600/10 rounded-2xl border border-cyan-500/20">
