...
export default function Contact() {
  ...
  const address = shop.data?.address;
  ...
  return <div className="editorial-page"><div className="store-container">
    ...
    {!shop.loading && !shop.error && <div className="contact-cards">
      ...
      <section className="information-card"><MapPin size={22} /><h2>Visit our shop</h2><p>Walk-in customers are welcome. Come in to explore phones and speak with us in person.</p><p>{address || 'Contact the shop for location and opening hours before visiting.'}</p>{address && <a href={`https://www.google.com/maps/search/?api=1&query=${encodeURIComponent(address)}`} target="_blank" rel="noreferrer">Find on Google Maps <ArrowUpRight size={16} /></a>}</section>
    </div>}
    {address && <section className="information-card shop-location" aria-labelledby="shop-location-heading">
      ...
      <iframe title="Afritek Gadget Spot shop location" src={`https://www.google.com/maps?q=${encodeURIComponent(address)}&output=embed`} width="100%" height="360" loading="lazy" referrerPolicy="no-referrer-when-downgrade" allowFullScreen style={{ border: 0, borderRadius: 8 }} />
      <a href={`https://www.google.com/maps/search/?api=1&query=${encodeURIComponent(address)}`} target="_blank" rel="noreferrer">Open shop location in Google Maps <ArrowUpRight size={16} /></a>
    </section>}
